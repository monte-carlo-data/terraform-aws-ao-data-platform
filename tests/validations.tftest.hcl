...
# random is deliberately NOT mocked: the ClickHouse passwords are generated by
# `ephemeral "random_password"`, and provider mocking does not support ephemeral
# resource types ("No ephemeral resource types in mock providers"). The real
# provider is safe here — it has no API and needs no credentials.
mock_provider "null" {}
...
# --- gated users create their secrets; generation is unconditional ---
#
# Generation is now `ephemeral "random_password"`, unconditional for the four
# always-on users and gated only on the Helm enabled flags for admin and
...
#
# Supplying clickhouse_passwords here also pins that the ephemeral variable
# still accepts ordinary values, which is what keeps existing callers working.

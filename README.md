...
- [bash](https://www.gnu.org/software/bash/) and [jq](https://jqlang.github.io/jq/) — only for `hack/verify-no-plaintext.sh`, the state verification step in the v3.0.0 upgrade
...
**No autoscaler — capacity is pre-provisioned.** This module does not install a cluster autoscaler, so every AZ a stateful pod lands in must have a node group standing by. When `create_vpc = true`, the AZs you list must be among the first `length(networking.private_subnet_cidrs)` of the region's available AZs — that is where the module places private subnets. A listed AZ with no matching private subnet fails the plan with the AWS provider's own "no matching EC2 Subnet found" error on `data.aws_subnet.dedicated_node_group_az_subnet["<az>"]`. One hint that error won't give you: `clickhouse_node_group.availability_zone`, left unset, defaults to the region's first AZ — a failure on an AZ you never explicitly configured usually means setting that variable instead.
...
This is no longer just an NLB-controller preference: the dedicated ClickHouse/Keeper node groups' subnet resolution (`data.aws_subnet.dedicated_node_group_az_subnet`) now enforces the same one-subnet-per-AZ requirement as a hard plan failure — two private subnets in one AZ fails the plan with "returned more than one result" rather than the node group silently spanning both. The only remediation is dropping the extra subnet, which is itself a `subnet_ids` change on any node group in that AZ, i.e. a node-group replacement — plan that separately if you hit it on a live deployment.
...
| `networking.private_subnet_cidrs` | `list(string)` | `["10.18.1.0/24", "10.18.2.0/24", "10.18.3.0/24"]` | Private subnet CIDRs for the created VPC; must sit inside `vpc_cidr`. Override together with it. Keep the list length at or below the region's available AZ count: the VPC submodule wraps subnet placement across AZs when there are more CIDRs than AZs, landing two subnets in one AZ — which the dedicated node groups' per-AZ subnet lookup now hard-fails on (see "NLB subnet placement is explicit"). |
...
| --- | --- | --- | --- |
| `clickhouse_write_only` | — | n/a | `bool`, default `false`. The per-deployment opt-in: it exists so deployments migrate individually. **Deprecated on arrival** — v4.0.0 will make the write-only path unconditional and remove both this flag and `clickhouse_passwords`. Must stay a statically known value. |
...
`clickhouse_password_versions` also serves the write-only path only: the version companion is rendered solely when the flag is set, and the variable has no effect on the legacy path. Unlike the two password variables, it carries **no validation** — setting it on the legacy path is accepted and silently has no effect.
...
> ```
>
> `otel` is the resource to inspect because it is not count-gated; the `admin` and `readonly_user` addresses need a `[0]` (as in the rotation check under [Rotating afterwards](#rotating-afterwards)).
>
...
>
> For that deployment the rest of this runbook reduces to: still complete steps 2–3 (the floor bumps and `init -upgrade` are not optional), then plan and confirm `No changes` — its passwords are already correct in Secrets Manager and its version counters are already at 1, so it needs no password supplied. Six in-place secret updates on that plan mean the flag did not take effect — stop and re-check, and apply nothing. Verify with step 6's script using an empty sentinel file. Steps 4, 5 and 7 do not apply.
>
...
```bash
STATE_JSON="$(mktemp)" SENTINELS="$(mktemp)"   # unpredictable names, created exclusively with mode 600
cat > "$SENTINELS"   # one password per line, then Ctrl-D

terraform show -json > "$STATE_JSON"
.terraform/modules/ao_data_platform/hack/verify-no-plaintext.sh \
  "$STATE_JSON" --sentinel-file "$SENTINELS"

rm -f "$STATE_JSON" "$SENTINELS"
```
...
> [!IMPORTANT]
> **Exit 0 covers the current state only.** Every prior state version — old object versions on a versioned S3 state bucket, old TFC/HCP state versions, a local `terraform.tfstate.backup` — still holds all six pre-migration passwords in plaintext, and a clean run of the script says nothing about them. Two follow-ups close that, either alone or together: purge or expire the old state versions, or treat the pre-migration passwords as exposed and rotate them once via `clickhouse_password_versions` now that the deployment is on the write-only path. This is also why step 4 preserves the current passwords rather than rotating them — the rotation is a deliberate follow-up, not a side effect of the migration.

**7. Remove the `clickhouse_passwords_wo` values you supplied in step 4** — the `.tfvars` entry, the environment variable, or the workspace variable. Later applies do not need them; the secrets are written, and nothing rewrites them until a version is bumped deliberately (see [Rotating afterwards](#rotating-afterwards)).
...
Rotation via `clickhouse_password_versions` applies to deployments on the write-only path (`clickhouse_write_only = true`). On the legacy path a version bump is accepted and silently does nothing — Terraform tracks `secret_string` directly there, so editing `clickhouse_passwords` is the lever, detected and written like any other argument.
...
`clickhouse_password_versions` is not a secret and is not transient: set it in the deployment's committed configuration, never as a run-scoped `TF_VAR_` or workspace variable you delete afterwards, and only ever increase it. Lowering or removing a bumped field is itself a change to `secret_string_wo_version` — which writes on *any* change, not only an increase — so it rewrites the secret with a newly generated password.
...
An unchanged version ID means no write happened — in practice, the footgun below. The version ID proves the write reached Secrets Manager, not that it reached ClickHouse: it flips the moment `PutSecretValue` returns, before ESO's next resync and the operator's `users.xml` re-render. **Do not retire the old credential until both are true: the version ID has changed, and the new password authenticates against ClickHouse** — a query as the rotated user, after ESO's resync interval.

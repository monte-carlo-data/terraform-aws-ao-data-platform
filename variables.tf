...
variable "helm" {
  description = <<-EOT
    ...
    var.clickhouse_passwords.admin — or var.clickhouse_passwords_wo.admin when
    clickhouse_write_only = true — or auto-generated.
    ...
    via var.clickhouse_passwords.readonly_user — or
    var.clickhouse_passwords_wo.readonly_user when clickhouse_write_only = true
    — or auto-generated.
...
variable "clickhouse_write_only" {
  description = <<-EOT
    Opt this deployment into the write-only ClickHouse password path: passwords
    are generated by `ephemeral "random_password"` and written through
    secret_string_wo, so no password reaches Terraform state or plan files
    (YET-2514).

    Defaults to false, which keeps the legacy path (managed random_password +
    secret_string, passwords in state). Upgrading to v3.0.0 therefore changes
    nothing until you set this — the flag exists so a fleet that shares one
    module pin can migrate one deployment at a time instead of all at once.

    Flipping it to true rewrites every ClickHouse secret on that apply, so you
    must supply the deployment's CURRENT passwords via clickhouse_passwords_wo
    in the SAME apply or each one is silently replaced by a freshly generated
    value. Read "Upgrading to v3.0.0" in this module's README before setting it.

    Once set, leave it set: it is the deployment's steady state, not a staging
    lever. Which variable serves which path:
    ...
    short-circuiting, which Terraform only does for a KNOWN predicate — wiring
    this to a data source or resource attribute makes the write-only locals fail
    at plan time.
  EOT
  type        = bool
  default     = false
...
variable "clickhouse_passwords" {
  description = <<-EOT
    Passwords for the ClickHouse SQL users on the LEGACY path only — i.e. when
    clickhouse_write_only = false (the default). Setting any field while
    clickhouse_write_only = true is REJECTED at plan time by the validation
    below, because on that path this variable is not read at all and silently
    ignoring it would rotate every password; use clickhouse_passwords_wo there.

    Every field is optional — any password left null is auto-generated.
    Supplied or generated, each password is stored in Secrets Manager and synced
    into the cluster by the External Secrets Operator; passwords never pass
    through Helm values.

    An empty string is treated as a supplied value and written through as-is,
    exactly as in v2.4.2. (The write-only path differs: there "" means
    "generate one".) Omit the field, or pass null, to auto-generate.
    ...
    Terraform state still contains the values — protect state accordingly. That
    is what the write-only path fixes; this variable cannot be ephemeral,
    because an ephemeral value may not feed secret_string, which is an ordinary
    (non-write-only) argument.
  ...
  # Cross-variable guard. On the write-only path this variable is never read, so
  # supplying it there is a silent no-op that regenerates all six passwords —
...
variable "clickhouse_passwords_wo" {
  description = <<-EOT
    Passwords for the ClickHouse SQL users on the WRITE-ONLY path only — i.e.
    when clickhouse_write_only = true. Setting any field while
    clickhouse_write_only = false (the default) is REJECTED at plan time by the
    validation below, for the same reason as its mirror on clickhouse_passwords:
    on that path this variable is not read; use clickhouse_passwords there.

    Same shape as clickhouse_passwords: every field is optional, and this is the
    variable you supply the deployment's CURRENT passwords through on the apply
    that opts into the write-only path. Semantics differ in one respect — a
    password left null OR set to the empty string is auto-generated here,
    whereas the legacy variable writes "" through as-is (v2.4.2 behavior). An
    empty ClickHouse password is never a legitimate input, and this path is new,
    so it takes the better semantic.

    Marked ephemeral as well as sensitive: values are omitted from Terraform
    state and plan files entirely, and redacted in plan/apply output and CI
    logs. Ephemeral is load-bearing rather than cosmetic — a CI pipeline that
    saves its plan (`-out`) and uploads it as an artifact would otherwise ship
    live ClickHouse passwords inside that plan file. Ephemeral variables accept
    ordinary values, so there is nothing special about how you supply one:
    a .tfvars file or TF_VAR_clickhouse_passwords_wo, never -var on a command
    line.

    Because the variable is ephemeral, Terraform requires it to be set again on
    `terraform apply <saved-plan>` if it was set during the plan — the value
    cannot be silently lost in a plan-then-apply split.
  ...
  # Mirror of the guard on clickhouse_passwords: supplying write-only passwords
...
variable "clickhouse_password_versions" {
  description = <<-EOT
    ...
    changes. This is the rotation lever.
    ...
    it writes every secret regardless, so you must supply your CURRENT passwords
    via clickhouse_passwords_wo in that same apply. Any user you omit has its
    password replaced by a freshly generated one, silently — the old value is
    unrecoverable once overwritten. Read "Upgrading to v3.0.0" in this module's
    README for the procedure before you run it.
  EOT
  type = object({
